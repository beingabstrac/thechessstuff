...
VIDEO_PATH = ROOT / "outputs" / "thechessstuff_mvp.mp4"
STORY_PATH = ROOT / "outputs" / "thechessstuff_storyboard.json"
...
def caption_for_story(story):
    p_num = story.get("puzzle_num", 1)
    p_date = story.get("date_str", story.get("date", "May 1, 2007"))
    p_title = story.get("title", "Daily Chess Puzzle")
    header = f"Chess #{p_num}  •  {p_date}\n{p_title}"
    tags = "#thechessstuff #chess #chesspuzzle #chessreels #tactics #reels"
    return f"{header}\n\nCan you spot the winning line before the timer runs out?\n\n{tags}"
...
def cloudinary_upload(video_path):
    ...
    boundary = f"----thechessstuff{int(time.time())}"
    chunks = []
    for name, val in fields.items():
        ...
        chunks.append(b"\r\n")
    data_bytes = Path(video_path).read_bytes()
    chunks.append(f"--{boundary}\r\n".encode())
    chunks.append(
...
def main():
    ...
    print(f"Posting reel to Buffer for Chess Puzzle '{story['title']}' ({story['difficulty']})...")
